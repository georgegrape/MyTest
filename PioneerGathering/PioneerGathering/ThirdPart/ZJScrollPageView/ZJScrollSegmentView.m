...
- (void)setupScrollLineAndCover {
    ...
    ZJCustomLabel *firstLabel = (ZJCustomLabel *)self.titleLabels[0];
    CGFloat coverX = firstLabel.zj_x+5;
    CGFloat coverW = firstLabel.zj_width-10;
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.frame = CGRectMake(coverX, self.zj_height - self.segmentStyle.scrollLineHeight-3, coverW, self.segmentStyle.scrollLineHeight);
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                weakSelf.scrollLine.zj_x = currentLabel.zj_x+5;
                weakSelf.scrollLine.zj_width = currentLabel.zj_width-10;
            } else {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    ...
                    weakSelf.scrollLine.zj_x = scrollLineX+ 5;
                    weakSelf.scrollLine.zj_width = scrollLineW-10;
                } else {
                    weakSelf.scrollLine.zj_x = currentLabel.zj_x+ 5;
                    weakSelf.scrollLine.zj_width = currentLabel.zj_width-10;
                }
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.zj_x = oldLabel.zj_x + xDistance * progress + 5;
            self.scrollLine.zj_width = oldLabel.zj_width + wDistance * progress - 10;
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                CGFloat oldScrollLineW = [self.titleWidths[oldIndex] floatValue] + wGap + 5;
                CGFloat currentScrollLineW = [self.titleWidths[currentIndex] floatValue] + wGap - 10;

...
@property (nonatomic, strong) GQSTextAndImageView *personsCountView;
...
- (void)setUpViews {
    ...
    GQSTextAndImageView *personsCountView = [[GQSTextAndImageView alloc] initWithFrame:CGRectZero Image:[UIImage imageNamed:@"textIcon"] Text:nil andExhibitDirection:GQSTextRightImageLeft isButton:NO];
    personsCountView.textFont = [UIFont systemFontOfSize:13*HEIGHT_SCALE];
    personsCountView.textColor = [UIColor lightGrayColor];
    [self.contentView addSubview:personsCountView];
    ...
    self.personsCountView = personsCountView;
...
- (void)layoutSubviews {
    ...
    [self.personsCountView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.dateView.mas_bottom).offset(5);
        make.left.equalTo(self.contentView.mas_left).offset(10*WIDTH_SCALE);
    }];
    
    [self.addressView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.personsCountView.mas_bottom).offset(5);
        make.left.equalTo(self.contentView.mas_left).offset(10*WIDTH_SCALE);
    }];
}
...
- (void)setModel:(BusinessOrMeditaitionModel *)model {
    ...
    NSString *personCount = [model.personsCount substringWithRange:NSMakeRange(2, model.personsCount.length - 3)];
    self.personsCountView.attributeText = [Util setWholeText:model.personsCount changeColorText:personCount withColor:[UIColor redColor]];
}
